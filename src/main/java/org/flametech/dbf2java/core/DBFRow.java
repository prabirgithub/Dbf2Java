...
import org.flametech.dbf2java.reader.MemoReader;
...
public class DBFRow {
    ...
    public DBFRow(byte[] source, DBFMetadata metadata, MemoReader memoReader, int recordNumber) {
    ...
    public DBFRow(byte[] source, DBFMetadata metadata, MemoReader memoReader, int recordNumber, Charset charset) {
        this.recordNumber = recordNumber;
        this.bytes = new byte[source.length];
        System.arraycopy(source, 0, this.bytes, 0, source.length);
        this.metadata = metadata;
        this.memoReader = memoReader;
        this.setStringCharset(charset);
    }
    ...
    */
    ...
    public String getString(String fieldName, Charset charset) {
        ...
        while ((actualLength > 0) && (bytes[actualOffset] == Dbf2JavaUtils.EMPTY)) {
            actualOffset++;
            actualLength--;
        }

        while ((actualLength > 0) && (bytes[actualOffset + actualLength - 1] == Dbf2JavaUtils.EMPTY)) {
            actualLength--;
        }
    ...
    public Date getDate(String fieldName) throws ParseException {
        ...
        return Dbf2JavaUtils.parseDate(s);
    }
